...
export default function MapForgottenScientist() {
  ...
  const [L, setL] = useState<typeof import("leaflet") | null>(null); // Leaflet state
  ...
  useEffect(() => {
    async function geocodeAll() {
      ...
      for (const s of rawScientists as Scientist[]) {
        ...
        const url = `https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(s.location)}`;
        try {
          const res = await fetch(url);
          const data = await res.json();
          if (data?.[0]) {
            results.push({ ...s, lat: parseFloat(data[0].lat), lng: parseFloat(data[0].lon) });
          } else {
  ...
  const createIcon = (img: string) => {
    if (!L) return undefined; // чекаємо на динамічний import
    return L.icon({ iconUrl: img, iconSize: [52, 52], iconAnchor: [26, 52], className: "custom-marker" });
  };

  const center: [number, number] = [20, 0];

  if (loadingGeo || !L)
    return <div className="flex items-center justify-center h-96 text-lg font-semibold">Geocoding scientist locations...</div>;
        ...
        <div className="relative rounded-2xl overflow-hidden shadow-xl">
          <aside className="absolute left-4 top-4 z-[1000] bg-white/80 backdrop-blur-lg p-4 rounded-2xl shadow">
            ...
            <div className="flex flex-col gap-2 text-sm">
              <select className="px-3 py-2 rounded-md border" value={fieldFilter ?? ""} onChange={(e) => setFieldFilter(e.target.value || null)}>
                <option value="">All Fields</option>
                {fields.map((f) => <option key={f}>{f}</option>)}
              </select>
              <select className="px-3 py-2 rounded-md border" value={countryFilter ?? ""} onChange={(e) => setCountryFilter(e.target.value || null)}>
                <option value="">All Countries</option>
                {countries.map((c) => <option key={c}>{c}</option>)}
              </select>
              <select className="px-3 py-2 rounded-md border" value={centuryFilter ?? ""} onChange={(e) => setCenturyFilter(e.target.value || null)}>
                <option value="">All Centuries</option>
                {centuries.map((c) => <option key={c}>{c}</option>)}
              </select>
              <button
                onClick={() => { setFieldFilter(null); setCountryFilter(null); setCenturyFilter(null); }}
          ...
          <div className="min-h-[620px] w-full">
            <MapContainer center={center} zoom={2} minZoom={2} scrollWheelZoom className="h-[620px] w-full" whenReady={() => setMapLoaded(true)}>
              <TileLayer url={"https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png"} />

              {filtered.map((s) => (
                <Marker key={s.id} position={[s.lat!, s.lng!]} icon={createIcon(s.image)} eventHandlers={{ mouseover: () => setHoverId(s.id), mouseout: () => setHoverId(null) }}>
                  <Popup>
                    <div className="text-center w-40">
                      <Image src={s.image} alt={s.name} width={64} height={64} className="rounded-full mx-auto mb-2" />
